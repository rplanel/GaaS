...
const resultRoutes = computed<Record<string, { tag: string, to: string }>>(() => {
  const analysisVal = toValue(analysisWithOutputsAndWorkflows)
...
<template>
  ...
  <slot v-else>
    ...
    <template v-else>
      ...
      <USeparator label="Jobs" />

      <UPageCard v-if="jobs && toolsObj" variant="ghost" :ui="{ container: 'lg:grid-cols-1' }">
